import { parse, visit, type ASTNode, Kind, type FieldNode, type OperationDefinitionNode, type SelectionSetNode } from 'graphql';
import type { SchemaData, SchemaDataStore, RootType, FieldWithDerivedData, GraphQLType, GraphQLField } from '../types/index';
 ...
 */
export function generateMockData(query: string, schemaData: SchemaDataStore | SchemaData): Record<string, any> {
    try {
    ...
    for (const selection of selectionSet.selections) {
        if (selection.kind === Kind.FIELD) {
            ...
            result[alias] = generateMockValue(fieldDef, fieldNode, schema);
             ...
             const fragmentSelections = generateMockForSelectionSet(selection.selectionSet, parentType, schema);
...
function generateMockValue(fieldDef: FieldWithDerivedData, fieldNode: FieldNode, schema: SchemaData): any {
