...
export const generatePostmanCollectionForQuery = (
    ...
    const collectionId = typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `col-${Math.random().toString(36).substring(2)}`;
    ...
    if (!headers['Content-Type'] && !Object.keys(headers).find(k => k.toLowerCase() === 'content-type')) {
        headerArray.push({
            key: 'Content-Type',
