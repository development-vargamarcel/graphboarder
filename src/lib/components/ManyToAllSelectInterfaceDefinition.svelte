<script lang="ts">
	...
	let mainWraperContext = getContext<QMSMainWraperContext>(
	...
	let inputFieldsContainerLocation = $derived(
		endpointInfo.get_inputFieldsContainerLocation(node, schemaData)
	);
	let inputFieldsContainer = $derived(
		getDeepField(node, inputFieldsContainerLocation, schemaData, 'inputFields')
	);
	let inputFieldsContainerRoot = $derived(
		getRootType(null, inputFieldsContainer.dd_rootName, schemaData)
	);
