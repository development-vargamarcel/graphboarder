<script lang="ts">
    ...
    const getKindColor = (kind: GraphQLKind) => {
        switch(kind) {
            ...
            case 'SCALAR': return 'badge-secondary';
            case 'INTERFACE': return 'badge-accent';
            case 'ENUM': return 'badge-info';
            case 'INPUT_OBJECT': return 'badge-warning';
            case 'UNION': return 'badge-error';
    ...
    <div class="w-1/3 border-r border-base-300 flex flex-col h-full">
        ...
        <div class="overflow-y-auto flex-1 p-2">
            {#each filteredTypes as type}
                <button
                    class="w-full text-left p-2 hover:bg-base-200 rounded flex justify-between items-center {selectedType?.name === type.name ? 'bg-primary text-primary-content hover:bg-primary-focus' : ''}"
        ...
        {#if selectedType}
            <div class="prose max-w-none">
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <span class="badge {getKindColor(selectedType.kind)} text-lg p-3">{selectedType.kind}</span>
                </div>
                                            ...
                                            {#if field.args && field.args.length > 0}
                                                <div class="text-xs text-gray-500 mt-1">
                                                    ...
                                                    {#each field.args as arg}
                                                        <span class="badge badge-ghost badge-sm mr-1">{arg.name}: {arg.dd_displayName}</span>
                                                    {/each}
                                                </div>
                                            {/if}
                                        </td>
