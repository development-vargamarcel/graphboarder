...
>
	<div
		class="bg-base-100 w-11/12 max-w-5xl h-[80vh] rounded-lg shadow-xl flex flex-col overflow-hidden"
	>
		<!-- Header -->
		...
		<div class="flex-grow overflow-y-auto p-4 bg-[#1e1e1e] text-gray-300 font-mono text-sm">
			...
			{#each filteredLogs as log (log.timestamp + Math.random())}
				<div
					class="mb-1 hover:bg-[#2d2d2d] p-1 rounded group flex items-start gap-2 break-all border-b border-gray-800"
				>
					<span class="text-gray-500 shrink-0 select-none text-xs w-[80px] text-right"
						>[{formatTime(log.timestamp)}]</span
					>

					{#if log.level === 'DEBUG'}
						<span class="text-gray-500 font-bold shrink-0 w-12 text-center select-none text-xs"
							>DEBUG</span
						>
					{:else if log.level === 'INFO'}
						<span class="text-blue-400 font-bold shrink-0 w-12 text-center select-none text-xs"
							>INFO</span
						>
					{:else if log.level === 'WARN'}
						<span class="text-yellow-400 font-bold shrink-0 w-12 text-center select-none text-xs"
							>WARN</span
						>
					{:else if log.level === 'ERROR'}
						<span class="text-red-500 font-bold shrink-0 w-12 text-center select-none text-xs"
							>ERROR</span
						>
					{/if}
					...
					<div class="flex-grow overflow-x-auto">
						{#each log.message as part}
							...
							{:else}
								<span class="mr-1 text-green-300 whitespace-pre-wrap font-mono text-xs"
									>{JSON.stringify(part, null, 2)}</span
								>
							{/if}
						{/each}
					</div>
