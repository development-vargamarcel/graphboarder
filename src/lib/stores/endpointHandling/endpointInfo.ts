	...
	typesExtraDataPossibilities: [
		{
			get_Val: () => {
				return {
					displayInterface: 'codeeditor' as const,
					defaultValue: '{}',
					use_transformerREVERSE: (val: any) => {
						return val;
					},
		...
		},
		{
			get_Val: () => {
				return {
					displayInterface: 'text' as const,
					defaultValue: ' ',
		...
		},
		{
			get_Val: () => {
				return {
					displayInterface: 'datetime-local' as const,
		...
		},
		{
			get_Val: () => {
				return {
					displayInterface: 'number' as const,
					defaultValue: 0,
					use_transformerREVERSE: (val: any) => {
						return val;
					},
		...
		},
		{
			get_Val: () => {
				return {
					displayInterface: 'boolean' as const,
					defaultValue: true,
					use_transformerREVERSE: (val: any) => {
						return val;
					},
				...
				return {
					displayInterface: 'ENUM' as const,
					defaultValue: [],
					use_transformerREVERSE: (val: any) => {
						return val;
					},
					use_transformer: (val: any) => {
						return val;
					}
				};
			},
...
export const create_endpointInfo_Store = (endpointConfiguration = {}) => {
	...
	const get_fieldsNames = (
		...
		for (const [name, possibilities] of Object.entries(storeVal[FieldsPossibleNamesName])) {
			fieldsNames[name] = (possibilities as any[]).find((possibility) =>
